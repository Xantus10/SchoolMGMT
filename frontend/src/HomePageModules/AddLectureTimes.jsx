    ...
    axios.get(process.env.REACT_APP_BE_ADDR+'/getLectureTimes', {headers: {"Content-Type": "application/json"}, withCredentials: true}).then(
      (resp) => {
        if (resp.data.status === 200) {
          const tmp = Array(10).fill(0).map((v, i) => <LectureTimeField key={i} alectureId={i} aClickable />);
          if (!checkNullArray(resp.data.times)) resp.data.times.forEach((time) => (tmp[time[0]]=<LectureTimeField key={time[0]} alectureId={time[0]} alectureTime={time[1]} aClickable />));
          setTimesList(tmp)
        } else {
